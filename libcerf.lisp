...
(defclcfun cerfcx (:struct cl_complex)
  ...
  ;; Compute erfcx(z) = exp(z^2) erfc(z), the complex
  ;; underflow-compensated complementary error function, trivially
  ;; related to Faddeeva's w_of_z.
  (return (faddeeva_w_of_z (complex (- (imagpart z))
                                    (realpart z)))))

(defclcfun cerfi (:struct cl_complex)
    ((var z (:struct cl_complex)))
  ;; Compute erfi(z) = -i erf(iz),
  ;; the rotated complex error function.
  (var e (:struct cl_complex)
       (cerf (complex (- (imagpart z))
                      (realpart z))))
  (return (complex (imagpart e)
                   (- (realpart e)))))

(defclcfun erfi :double
    ((var x :double))
...
(defclcfun dawson :double
    ((var x :double))
  ;; Compute dawson(x) = sqrt(pi)/2 * exp(-x^2) * erfi(x),
  ;; Dawson's integral for a real argument.
  (var spi2 (const :double)
       "0.8862269254527580136490837416705725913990"); sqrt(pi)/2
  (return (* spi2
             (faddeeva_im_w_of_x x))))

(defclcfun faddeeva_re_w_of_z :double
    ((var x :double)
     (var y :double))
  (return (realpart (faddeeva_w_of_z (complex x y)))))

(defclcfun faddeeva_im_w_of_z :double
    ((var x :double)
     (var y :double))
  (return (imagpart (faddeeva_w_of_z (complex x y)))))
...
// Regular case, both parameters are nonzero"
  ...
  (setf (member z :imag)
        (/ gam (sqrt 2d0) sig))
  (return (/ (realpart (faddeeva_w_of_z z))
             s2pi
             sig))
  "} }")
...
// using w_of_z except for certain regions."
  ...
  (when (zerop y)
    (return (complex (erf x)
                     y))) ; preserve sign of 0
  (when (zerop x) ; handle separately for speed & handling of y = Inf or NaN
    (return
      (complex x ;; preserve sign of 0
...
(defclcfun cerfc (:struct cl_complex)
    ((var z (:struct cl_complex)))
  "// Steven G. Johnson, October 2012.
...
(defclcfun cerf_w_im_y100 :double
  ...
  ;; Gonna cheat here and just insert raw C code
  "    switch ((int) y100) {
    case 0: {
        double t = 2*y100 - 1;
...
(defclcfun faddeeva_im_w_of_x :double
    ((var x :double))
  (var ispi (const :double)
       0.56418958354775628694807945156d0); 1 / sqrt(pi)
  (if (>= x 0)
      (progn
        (if (> x 45)
            (progn
...
;;; w_of_z.c
(defclcfun cerf_sinc :double
    ((var x :double)
     (var sinx :double))
  (return (? (< (fabs x) 1d-4)
             (- 1d0
                (* 0.1666666666666666666667d0 x x))
             (/ sinx x))))

(defclcfun cerf_sinh_taylor :double
    ((var x :double))
  ...
  (return
    (* x
       (+ 1
          (* x x
             (+ 0.1666666666666666666667d0
                (* 0.00833333333333333333333d0 x x)))))))

(defclcfun cerf_sqr :double
    ((var x :double))
  (return (* x x)))

(defclcfun faddeeva_w_of_z (:struct cl_complex)
    ((var z (:struct cl_complex)))
  ;; Steven G. Johnson, October 2012.
  ...
  (when (zerop (realpart z))
    ;; Purely imaginary input, purely real output.
    ;; However, use creal(z) to give correct sign of 0 in cimag(w).
    (return (complex (erfcx (imagpart z))
                     (realpart z))))
  (when (zerop (imagpart z))
    ;; Purely real input, complex output.
    (return (complex (exp (- (cerf_sqr (realpart z))))
                     (faddeeva_im_w_of_x (realpart z)))))
  ...
  (var ret (:struct cl_complex)) ; return value
  (setf (member ret :real) 0d0)
  (setf (member ret :imag) 0d0)
  ...
  (var xs :double
       (? (< y 0)
          (- (realpart z))
          (realpart z))); // compute for -z if y < 0
  "if (x + ya > 4000) { // nu <= 2
  ...
  (setf ret (complex (* denom yax)
                     denom))
  "}
  else if (isinf(ya)) {
  return ((isnan(x) || y < 0)
          ? complex(NAN,NAN) : complex(0,0));
  }
  else {
  double xya = xs / ya;
  ...
  (setf sum5
        (+ sum5
           (* coef
              ...
              a
              n
              (cerf_sinh_taylor (* 2 a  n x)))))

  (concat "// test convergence via sum3
                   ...
                   for (int n = 1; 1; ++n) {
                            ...
                            ret = complex((expx2erfcxy - c*y*sum1) * cos(2*x*y)
                            + (c*x*expx2) * sinxy * " (cerf_sinc "x*y" "sinxy") ",0);
                            }
                            else {
                            double xs = realpart(z);
                            ...
                            ret = complex(coef1 * cos2xy + coef2 * sinxy * cerf_sinc(xs*y, sinxy),
                                          coef2 * cerf_sinc(2*xs*y, sin2xy) - coef1 * sin2xy);
                            }
                            }
                            ...
                            ret = complex(exp(-x*x),0); // |y| < 1e-10, so we only need exp(-x*x) term
                            // (round instead of ceil as in original paper; note that x/a > 1 here)
                                      ...
                                      for (dn = 1; n0 - dn > 0; ++dn) { // loop over n0-dn and n0+dn terms
                                              ...
                                              double tp = exp(-cerf_sqr(a*dn+dx));
                                              ...
                                              double np = n0 + dn++;
                                              double tp = exp(-cerf_sqr(a*dn+dx)) / (a2*(np*np) + y*y);
                                              sum3 += tp;
                                              sum5 += a * np * tp;
